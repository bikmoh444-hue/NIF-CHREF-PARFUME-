  ...
  return (
    <article className="group">
      <Link href={`/produit/${product.slug}`} className="product-frame relative block aspect-[4/4.6] overflow-hidden">
        {product.is_bestseller && <span className="absolute left-3 top-3 z-10 bg-gold px-3 py-1 text-xs font-bold text-white">BEST SELLER</span>}
      ...
      <div className="mt-3 flex items-start justify-between gap-3">
        ...
        <AddToCartButton item={{ id: product.id, slug: product.slug, name: product.name_fr, price: Number(product.price), image_url: product.image_url, kind: "product" }} label="ADD" className="px-3 py-2 text-xs" />
      </div>
    </article>
  );
}
